/* Change History          */
/* V1.2 - Add zero padding */

using Toybox.Math;
using Toybox.System;

class OsGridRefUtils
{
    var foreign = "Outside UK?";
    var valid = false;
    var text = "Unknown!";       // Grid ref square - e.g. SU
  ...
  function initialize(lat, lon, p )
  {
    ...
    precision = p;
    valid = isValidUKLocation(lat, lon);
    if (valid == true) {
...
// http://www.jstott.me.uk/jscoord/  -  toSixFigureString()
//
...
//System.println("numeric grid "  + east.toString() + "," + north.toString() );
      ...
      if (valid == true && east >= 0 && north >= 0)
      {
        ...
        if (index >= 0 && index < alpha.size() )
        {
          ...
          {
             var format_string = "%0" + precision/2 + "u";                //* V1.2 *
             e = (east - (100000 * hundredkmE)).format(format_string);    //* V1.2 *
             n = (north - (100000 * hundredkmN)).format(format_string);   //* V1.2 *
          }
          valid = true;
...
// Create grid ref from WSG84 lat /long
function isValidUKLocation(lat, lon)
{
